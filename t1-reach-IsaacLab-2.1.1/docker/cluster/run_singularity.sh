...
singularity exec \
    ...
    -B $TMPDIR/$dir_name:/workspace/isaaclab:rw \
    --nv --containall $TMPDIR/$2.sif \
    bash -c "export ISAACLAB_PATH=/workspace/isaaclab && cd /workspace/isaaclab/diffusion_humanoid && export WANDB_API_KEY=${WANDB_KEY} && /isaac-sim/python.sh ${CLUSTER_PYTHON_EXECUTABLE} ${@:3}"
else
