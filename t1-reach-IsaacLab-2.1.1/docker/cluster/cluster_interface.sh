...
case $command in
    ...
    job)
        ...
        if [ "$profile" = "dpft" ] || [ "$profile" = "dpftcloud" ]; then
            ...
            rsync -rh --exclude="*.git*" /$SCRIPT_DIR/../../../diffusion_humanoid $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR
            echo "[INFO] Syncing t1_dp_tasks code..."
            rsync -rh  --exclude="*.git*" /$SCRIPT_DIR/../../../t1_dp_tasks $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR
            rsync -rh  --exclude="*.git*" /$SCRIPT_DIR/../../../ckpts $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR/diffusion_humanoid
        fi
        # execute job script
