...
submit_job() {
    ...
    case $CLUSTER_JOB_SCHEDULER in
        "SLURM")
            CMD='bash -l'
            job_script_file=submit_job_slurm_dpft.sh
            ;;
        "PBS")
            CMD=bash
...
help() {
    ...
    echo -e "  log                           Sync logs from the cluster."
    echo -e "\nwhere:"
...
case $command in
    ...
    job)
        ...
        # Check if profile is dpft or dpftcloud and sync additional directories
        if [ "$profile" = "dpft" ] || [ "$profile" = "dpftcloud" ]; then
            echo "[INFO] Syncing diffusion_humanoid code..."
            rsync -rh --exclude="*.git*" --exclude="output*/" /$SCRIPT_DIR/../../../diffusion_humanoid $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR
            echo "[INFO] Syncing t1_dp_tasks code..."
            rsync -rh  --exclude="*.git*" /$SCRIPT_DIR/../../../t1_dp_tasks $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR
            echo "[INFO] Syncing Checkpoints..."
            rsync -rh  --exclude="*.git*" /$SCRIPT_DIR/../../../ckpts/ $CLUSTER_LOGIN:$CLUSTER_CHECKPOINTS_DIR/
        fi
        # execute job script
        ...
        ;;
    log)
        echo "[INFO] Syncing logs from cluster"
        source $SCRIPT_DIR/.env.dpft
        rsync -rh $CLUSTER_LOGIN:$CLUSTER_ISAACLAB_DIR/logs/ /$SCRIPT_DIR/../../logs/
        ;;
    *)
