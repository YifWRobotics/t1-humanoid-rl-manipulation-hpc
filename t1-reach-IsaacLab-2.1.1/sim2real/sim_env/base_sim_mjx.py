...
class BaseSimulatorMJX:
    ...
    def init_scene(self):
        ...
        self.mj_model = mujoco.MjModel.from_xml_path(
            self.config["ROBOT_SCENE"])
        self.mj_data = mujoco.MjData(self.mj_model)
        self.mj_model.opt.timestep = self.sim_dt

        self.mjx_model = mjx.put_model(self.mj_model)
        # self.mjx_model.opt.timestep = self.mj_model.opt.timestep
        self.mjx_data = mjx_env.init(self.mjx_model)
        partial_step = partial(mjx.step, self.mjx_model)
        self.jit_step = jax.jit(partial_step)
    ...
    def sim_step(self):
        ...
        # Handle elastic band force application
    ...
    def simulation_thread(self):
        ...
        while self.viewer.is_running():
            ...
            if sim_cnt % (self.viewer_dt / self.sim_dt) == 0:
                self.mj_data.qpos = self.mjx_data.qpos
...
if __name__ == "__main__":
    ...
    simulation = BaseSimulatorMJX(config)
