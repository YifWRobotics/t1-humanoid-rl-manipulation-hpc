...
class LocoManipPolicy(DecLocomotionPolicy):
    def __init__(
        ...
        # --- DEBUG what config this process actually loaded ---
        try:
            obs_list = self.config["obs_dict"]["actor_obs"]
            per_frame = sum(self.obs_dims[k] for k in obs_list)
            print("\n[DEBUG] Using config file:", os.path.abspath(self.config_path) if "config_path" in self.__dict__ else "(path unknown)")
            print("[DEBUG] actor_obs order:", obs_list)
            print("[DEBUG] obs_dims subset:", {k: self.obs_dims[k] for k in obs_list})
            print("[DEBUG] Computed per-frame dim:", per_frame, "\n")
        except Exception as e:
            print("[DEBUG] Cannot compute per-frame dims:", e)
    ...
    def get_current_obs_buffer_dict(self, robot_state_data):
        ...
        # 🔻 REMOVE anything your ONNX wasn’t trained on
        for k in ("phase_time", "sin_phase", "cos_phase", "ref_upper_dof_pos"):
            current_obs_dict.pop(k, None)
    ...
    def rl_inference(self, robot_state_data):
        ...
        # Keep a 12-wide copy for observations/history
        self.last_policy_action = policy_action.copy()

        # Scale the leg actions
        scaled_policy_action = policy_action * self.policy_action_scale

        # ---- Expand to full 29-DOF action vector ----
        full_action = np.zeros((1, self.num_dofs), dtype=np.float32)

        # Build list of lower-body DOF indices WITHOUT the waist
        try:
            waist_idx = self.dof_names.index("Waist")
        except ValueError:
            waist_idx = None

        lower_no_waist = (
            [idx for idx in self.lower_dof_indices if idx != waist_idx]
            if self.lower_dof_indices
            else list(range(17, 29))  # fallback for T1: 17..28
        )

        # Safety: ensure we’re assigning exactly the number of columns we have
        if len(lower_no_waist) != scaled_policy_action.shape[1]:
            if len(lower_no_waist) > scaled_policy_action.shape[1]:
                # Trim index list down to match the 12 columns we have
                lower_no_waist = lower_no_waist[: scaled_policy_action.shape[1]]
            else:
                # Pad the action array to match the index list length
                pad = np.zeros((1, len(lower_no_waist) - scaled_policy_action.shape[1]), dtype=np.float32)
                scaled_policy_action = np.concatenate([scaled_policy_action, pad], axis=1)

        # Drop the leg actions into their actual joint slots
        full_action[:, lower_no_waist] = scaled_policy_action

        # Optionally add residuals on upper body (as offsets around defaults)
        if self.residual_upper_body_action and len(self.upper_dof_indices) > 0:
            full_action[:, self.upper_dof_indices] += (
                self.ref_upper_dof_pos - self.default_dof_angles[self.upper_dof_indices]
            )

        # Return full 29-DOF action vector
        return full_action         
