...
class Camera(SensorBase):
        ...
        for cam_prim_path in self._view.prim_paths:
            ...
            for name in self.cfg.data_types:
                ...
                try:
                    ...
                    # add to registry only if attachment succeeded
                    self._rep_registry[name].append(rep_annotator)
                except (TypeError, RuntimeError) as e:
                    # Handle known issues with annotator attachment (e.g., dtype errors in Isaac Sim)
                    carb.log_warn(
                        f"Failed to attach annotator '{annotator_name}' to render product '{render_prod_path}': {e}. "
                        f"This annotator will be skipped. Camera data type '{name}' may not be available."
                    )
                    # Continue without adding to registry - this annotator will not be available
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        if len(self._data.output) == 0:
            ...
            for name, annotators in self._rep_registry.items():
                # Skip if no annotators were successfully attached for this data type
                if len(annotators) == 0:
                    continue
                # Check if we have enough annotators for all environments
                if len(annotators) < self._num_envs:
                    carb.log_warn(
                        f"Not enough annotators for data type '{name}': expected {self._num_envs}, "
                        f"got {len(annotators)}. Skipping data update for this type."
                    )
                    continue
    ...
    def _create_annotator_data(self):
        ...
        for name, annotators in self._rep_registry.items():
            ...
            if len(annotators) == 0:
                carb.log_warn(
                    f"No annotators available for data type '{name}'. Skipping buffer creation for this type."
                )
                continue
            # Check if we have enough annotators for all environments
            if len(annotators) < self._num_envs:
                carb.log_warn(
                    f"Not enough annotators for data type '{name}': expected {self._num_envs}, "
                    f"got {len(annotators)}. Skipping buffer creation for this type."
                )
                continue
