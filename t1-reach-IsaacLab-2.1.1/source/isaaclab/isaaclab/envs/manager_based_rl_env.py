...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: ManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.base_acc_data_list = []
        self.base_vel_data_list = []
        self.base_pose_data_list = []
    ...
    def load_managers(self):
        ...
        # Create a separate reward manager for RL (low-level) rewards if configured
        if hasattr(self.cfg, 'rl_rewards') and self.cfg.rl_rewards is not None:
            self.rl_reward_manager = RewardManager(self.cfg.rl_rewards, self)
            print("[ManagerBasedRLEnv] Created separate RL reward manager separation separate from DP reward manager")
        else:
            self.rl_reward_manager = None
    ...
    def get_phase(self) -> torch.Tensor:
        """Get the phase of the environment. Phase \in [0, 1)."""
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for _ in range(self.cfg.decimation):
            ...
            # # collect base data
            # base_acc_data = self.scene["robot"].data.body_acc_w[:, 0, :].cpu().numpy()
            # self.base_acc_data_list.append(base_acc_data)

            # base_vel_data = self.scene["robot"].data.body_vel_w[:, 0, :].cpu().numpy()
            # self.base_vel_data_list.append(base_vel_data)

            # base_pose_data = self.scene["robot"].data.body_pose_w[:, 0, :].cpu().numpy()
            # self.base_pose_data_list.append(base_pose_data)
        ...
        self.reward_buf = self.reward_buf.clamp(min=0)
        
        # Compute RL (low-level) rewards if configured
        if self.rl_reward_manager is not None:
            rl_reward = self.rl_reward_manager.compute(dt=self.step_dt)
            # rl_reward = rl_reward.clamp(min=0)
            
            # Add RL reward to extras
            if self.extras is None:
                self.extras = {}
            self.extras['rl_reward'] = rl_reward
    ...
    def render(self, recompute: bool = False) -> np.ndarray | None:
        ...
        if self.render_mode == "human" or self.render_mode is None:
            ...
            if not hasattr(self, "_rgb_annotator"):
                ...
                self._rgb_annotator_attached = False
                try:
                    self._rgb_annotator.attach([self._render_product])
                    self._rgb_annotator_attached = True
                except (TypeError, RuntimeError) as e:
                    # Handle Isaac Sim rendering initialization errors
                    # This can happen if the render product isn't ready yet or there's a dtype issue
                    import warnings
                    warnings.warn(f"Failed to attach RGB annotator: {e}. Rendering may not work correctly.")
                    self._rgb_annotator_attached = False
            # Check if annotator is attached before getting data
            if not getattr(self, "_rgb_annotator_attached", False):
                # Return black image if annotator failed to attach
                return np.zeros((self.cfg.viewer.resolution[1], self.cfg.viewer.resolution[0], 3), dtype=np.uint8)
            # obtain the rgb data
            try:
                rgb_data = self._rgb_annotator.get_data()
            except Exception as e:
                # If get_data fails, return black image
                import warnings
                warnings.warn(f"Failed to get RGB data from annotator: {e}. Returning black image.")
                return np.zeros((self.cfg.viewer.resolution[1], self.cfg.viewer.resolution[0], 3), dtype=np.uint8)
