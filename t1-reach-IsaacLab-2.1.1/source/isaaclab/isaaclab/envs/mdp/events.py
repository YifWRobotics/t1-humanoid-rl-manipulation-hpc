        ...
        for i, env_id in enumerate(env_ids):
            ...
            if not has_scale_attr:
                ...
                if op_order_spec is None:
                    op_order_spec = Sdf.AttributeSpec(prim_spec, UsdGeom.Tokens.xformOpOrder, Sdf.ValueTypeNames.TokenArray)
...
class randomize_rigid_body_mass_class(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if not isinstance(self.asset, (RigidObject, Articulation)):
            raise ValueError(
                f"Randomization term 'randomize_rigid_body_mass' not supported for asset: '{self.asset_cfg.name}' with type: '{type(self.asset)}'."
            )
        ...
        self.mass_noise = torch.zeros((env.scene.num_envs, len(self.body_ids)), dtype=torch.float32, device="cpu")
    ...
    def __call__(
        ...
        masses = _randomize_prop_by_op(masses, mass_distribution_params, env_ids, self.body_ids, operation=operation, distribution=distribution)
        ...
        if recompute_inertia:
            ...
            if isinstance(self.asset, Articulation):
                ...
                inertias[env_ids[:, None], self.body_ids] = self.asset.data.default_inertia[env_ids[:, None], self.body_ids] * ratios[..., None]
            else:
...
class randomize_rigid_body_com_class(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        self.com_noise = torch.zeros((env.scene.num_envs, len(self.body_ids), 3), dtype=torch.float32, device="cpu")
...
def randomize_joint_parameters(
    ...
    if friction_distribution_params is not None:
        ...
        asset.write_joint_friction_coefficient_to_sim(friction_coeff[env_ids[:, None], joint_ids], joint_ids=joint_ids, env_ids=env_ids)
    ...
    if lower_limit_distribution_params is not None or upper_limit_distribution_params is not None:
        ...
        asset.write_joint_position_limit_to_sim(joint_pos_limits, joint_ids=joint_ids, env_ids=env_ids, warn_limit_violation=False)
...
def randomize_fixed_tendon_parameters(
    ...
    if lower_limit_distribution_params is not None or upper_limit_distribution_params is not None:
        ...
        if (tendon_limits[..., 0] > tendon_limits[..., 1]).any():
            raise ValueError(
                "Randomization term 'randomize_fixed_tendon_parameters' is setting lower tendon limits that are greater than upper tendon limits."
            )
...
def reset_root_state_from_terrain(
    ...
    * :attr:`velocity_range` - a dictionary of velocity ranges for each axis and rotation. The keys of the dictionary
    ...
    if valid_positions is None:
        raise ValueError(
            f"The event term 'reset_root_state_from_terrain' requires valid flat patches under 'init_pos'. Found: {list(terrain.flat_patches.keys())}"
        )
...
def reset_and_target_joints_by_scale(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    position_range: tuple[float, float],
    velocity_range: tuple[float, float],
    ...
    asset.set_joint_position_target(joint_pos, env_ids=env_ids, joint_ids=asset_cfg.joint_ids)
...
def reset_robot_upper_joints_by_offset_default(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    position_range: tuple[float, float],
    velocity_range: tuple[float, float],
    ...
    """Reset the designated upper body joints by adding offsets to default angles and clamping within soft limits.

    This function samples random offsets from the given ranges and adds them to the default joint positions
    and velocities. The resulting values are then clamped to the soft joint limits before being set into
    ...
    Args:
        ...
        env_ids: The environment indices to reset.
        asset_cfg: SceneEntityCfg for the robot asset.
        position_range: Tuple of (min, max) offset range for joint positions.
        velocity_range: Tuple of (min, max) offset range for joint velocities.
    """
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    joint_indices = asset_cfg.joint_ids

    # get default joint state for the selected joints (use two-step indexing to avoid broadcasting issues)
    default_joint_pos = asset.data.default_joint_pos[env_ids][:, joint_indices].clone()
    default_joint_vel = asset.data.default_joint_vel[env_ids][:, joint_indices].clone()

    # add random offsets to default positions and velocities
    target_pos = default_joint_pos + math_utils.sample_uniform(*position_range, default_joint_pos.shape, default_joint_pos.device)
    target_vel = default_joint_vel + math_utils.sample_uniform(*velocity_range, default_joint_vel.shape, default_joint_vel.device)

    # clamp joint pos to soft limits (use two-step indexing)
    joint_pos_limits = asset.data.soft_joint_pos_limits[env_ids][:, joint_indices, :]
    target_pos = target_pos.clamp_(joint_pos_limits[..., 0], joint_pos_limits[..., 1])
    # clamp joint vel to soft limits (use two-step indexing)
    joint_vel_limits = asset.data.soft_joint_vel_limits[env_ids][:, joint_indices]
    target_vel = target_vel.clamp_(-joint_vel_limits, joint_vel_limits)
...
###### A predefined preparation state for a humanoid robot (e.g., Franka Emika Panda, iCub, etc.)
...
PREP_STATE = ArticulationCfg.InitialStateCfg(
    ...
    joint_pos={
        "AAHead_yaw": 0.0,
        "Head_pitch": 0.0,
        "Left_Shoulder_Pitch": 0.0706,
        "Right_Shoulder_Pitch": 0.0706,
...
def reset_to_prep(env: ManagerBasedEnv, env_ids: torch.Tensor, asset_cfg: SceneEntityCfg):
    """Reset robot joints to preparation state (only for the specified env_ids)."""
    asset: Articulation = env.scene[asset_cfg.name]

    # work on subset only
    joint_pos_sub = asset.data.joint_pos[env_ids].clone()
    joint_vel_sub = torch.zeros_like(asset.data.joint_vel[env_ids])

    for joint_name, target_pos in PREP_STATE.joint_pos.items():
        # find_joints may return (indices,) or (indices, names)
        res = asset.find_joints(joint_name, preserve_order=True)
        ...
        if not joint_indices:
            continue
        joint_indices = torch.as_tensor(joint_indices, device=joint_pos_sub.device, dtype=torch.long)
        # assign scalar target_pos to all selected environments for these joints
        joint_pos_sub[:, joint_indices] = float(target_pos)

    # write only the subset
    asset.write_joint_state_to_sim(joint_pos_sub, joint_vel_sub, env_ids=env_ids)
...
class randomize_visual_texture_material(ManagerTermBase):
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            self.material_prims = rep.functional.create_batch.material(mdl="OmniPBR.mdl", bind_prims=prims_group, count=num_prims, project_uvw=True)
    ...
    def __call__(
        ...
        if compare_versions(version, "1.12.4") < 0:
            ...
            texture_rotation = texture_rotation if texture_rotation else self._cfg.params.get("texture_rotation", (0.0, 0.0))
...
def _randomize_prop_by_op(
    ...
    if distribution == "uniform":
        ...
        raise NotImplementedError(
            f"Unknown distribution: '{distribution}' for joint properties randomization. Please use 'uniform', 'log_uniform', 'gaussian'."
        )
