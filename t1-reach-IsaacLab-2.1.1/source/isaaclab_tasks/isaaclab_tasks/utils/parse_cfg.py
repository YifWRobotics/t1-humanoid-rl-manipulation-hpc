...
def get_checkpoint_path(
    ...
    try:
        entries = [run for run in os.scandir(log_path)]
    except FileNotFoundError as exc:
        raise ValueError(f"Log directory: '{log_path}' does not exist.") from exc

    runs = [os.path.join(log_path, run.name) for run in entries if run.is_dir() and re.match(run_dir, run.name)]
    # allow the log directory itself to be used when no nested run folders exist
    if len(runs) == 0 and re.match(run_dir, os.path.basename(os.path.normpath(log_path))):
        runs = [os.path.normpath(log_path)]
    if len(runs) == 0:
        available_runs = sorted([run.name for run in entries if run.is_dir()])
        extra = f" Available runs: {', '.join(available_runs)}." if available_runs else " Directory is empty."
        raise ValueError(f"No runs present in the directory: '{log_path}' match: '{run_dir}'.{extra}")
